...
- `pip install docxcompose docxtpl htmldocx pypdf`
...
In PDF mode, the template also exposes `register_pdf` so you can merge additional PDF files with the PDF produced from your DOCX (for example cover pages or terms appended after the report). The main report PDF sits between any PDFs registered with `position='before'` and those with `position='after'` (or the default). Only valid PDF data is accepted; register one PDF per call.
